...
class PetFriends:
    ...
    #     """Метод делает запрос к API сервера и возвращает статус запроса и результат в формате JSON
    #     со списком собственных питомцев питомцев, совпадающих с фильтром 'my_pets'.
    #     """
    #
    #     headers = {'auth_key': auth_key['key']}
    #     filter = {'filter': filter}
    ...
    def add_photo_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет (постит) на сервер запрос о питомце по его pet_id, добавляет фото и возвращает статус
